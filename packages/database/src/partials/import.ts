...
export const flowImportMetaSchema = z.object({
  ...
  // Localized audit-detail template (generated by the builder action with the
  // actor's locale). The worker replaces the `{name}` placeholder with the
  // imported flow's name — the worker has no request locale of its own.
  auditDetailTemplate: z.string().optional(),
})
